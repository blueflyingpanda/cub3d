...
/*   Updated: 2020/09/01 19:08:01 by lhelper          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_map(t_data *img, char *line, int *y, t_flags *flag)
{
	...
	while (line[cord.x])
	{
		if (line[cord.x] == '1')
		{
			img->obs[img->i].x = cord.x;
			img->obs[img->i].y = cord.y;
			img->i++;
		}
		else if (line[cord.x] == '2')
		{
			img->items[img->sp].x = cord.x;
			img->items[img->sp].y = cord.y;
			img->sp++;
		}
